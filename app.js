...
// Debounce genérico — evita re-renderizar a lista inteira a cada tecla digitada
// nas buscas (otimização de desempenho, sem remover nenhuma funcionalidade)
...
const renderOSDebounced        = debounce(() => renderOS());
const renderClientesDebounced  = debounce(() => renderClientes());
const renderEstoqueDebounced   = debounce(() => renderEstoque());
const globalSearchDebounced    = debounce(q => globalSearch(q), 180);
const filtrarClienteOSDebounced= debounce(q => filtrarClienteOS(q), 180);
...
// ══════════════════════════════════════════════════════════════
// CAPTURA DO COMPROVANTE (html2canvas) — helper único
// FIX: alinhamento quebrado em PDF/imagem tinha 3 causas:
//  1. html2canvas não renderiza corretamente a propriedade CSS "gap"
//     em flex/grid (bug conhecido da biblioteca) — os elementos do
//     comprovante usam "gap" para espaçamento, então na tela (navegador)
//     fica alinhado, mas na captura (canvas) os espaçamentos somem e
//     tudo se sobrepõe/desalinha. Corrigido injetando, só na cópia
//     usada pela captura (onclone), um CSS que troca "gap" por margin.
//  2. Fontes web podem não estar 100% carregadas no instante da
//     captura, mudando a métrica do texto e desalinhando linhas.
//     Corrigido aguardando document.fonts.ready.
//  3. Em telas de celular estreitas (<480px) o layout muda para 1
//     coluna (media query), então gerar o comprovante pelo celular
//     produzia um recibo diferente do "oficial" 2 colunas. Corrigido
//     forçando a captura numa largura fixa (windowWidth), independente
//     do tamanho da tela do aparelho.
// ══════════════════════════════════════════════════════════════
const COMP_EXPORT_CSS = `
  .comp-top-left, .comp-top-right, .comp-meta, .comp-footer-box { gap:0 !important; }
  .comp-top-left > * + *  { margin-top:4px !important; }
  .comp-top-right > * + * { margin-top:8px !important; }
  .comp-meta > * + *      { margin-top:3px !important; }
  .comp-meta span         { gap:0 !important; }
  .comp-meta span > * + * { margin-left:5px !important; }
  .comp-card-title        { gap:0 !important; }
  .comp-card-title > * + *{ margin-left:7px !important; }
  .comp-cards             { gap:0 !important; display:flex !important; align-items:stretch !important; }
  .comp-cards > .comp-card{ flex:1 1 0; }
  .comp-cards > .comp-card:first-child { margin-right:14px !important; }
  .comp-garantia-box      { gap:0 !important; }
  .comp-garantia-box > * + * { margin-left:10px !important; }
  .comp-footer            { gap:0 !important; display:flex !important; align-items:stretch !important; }
  .comp-footer > .comp-footer-box { flex:1 1 0; }
  .comp-footer > .comp-footer-box:first-child { margin-right:16px !important; }
  .comp-footer-box > * + * { margin-top:6px !important; }
  .comp-footer-title      { gap:0 !important; }
  .comp-footer-title > * + * { margin-left:6px !important; }
  .comp-bottom            { gap:0 !important; }
  .comp-bottom-item       { gap:0 !important; margin-right:24px !important; }
  .comp-bottom-item > * + * { margin-left:5px !important; }
  .comp-photos            { gap:0 !important; display:flex !important; flex-wrap:wrap !important; }
  .comp-photos img        { width:calc(33.333% - 4px) !important; margin:2px !important; }
`;

async function _waitImagesLoaded(el) {
...
async function capturarComprovante(el) {
  if (document.fonts && document.fonts.ready) { try { await document.fonts.ready; } catch {} }
  await _waitImagesLoaded(el);
  return html2canvas(el, {
    scale: 2,
    backgroundColor: '#ffffff',
    useCORS: true,
    logging: false,
    windowWidth: 700,   // força layout "desktop" (2 colunas) mesmo capturando de um celular estreito
    onclone(clonedDoc) {
      const style = clonedDoc.createElement('style');
      style.textContent = COMP_EXPORT_CSS;
...
async function compartilharComp() {
  ...
  try {
    ...
    const canvas = await capturarComprovante(el);
...
function novoCliente() {
  ['form-cli-id','form-cli-nome','form-cli-tel','form-cli-doc','form-cli-email','form-cli-aniv',
   'form-cli-cep','form-cli-rua','form-cli-num','form-cli-bairro','form-cli-cidade','form-cli-uf'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  const st = document.getElementById('form-cli-cep-status'); if (st) st.textContent = '';
...
// Feature: endereço estruturado (CEP → cidade/UF automáticos) — #cep-autocomplete
function maskCEP(el) {
...
// Preenche rua/numero/bairro/cidade/UF a partir de um endereço já salvo (edição)
// Aceita o formato gerado por montarEnderecoCliente(); se não reconhecer o
// padrão (endereço antigo digitado livremente), joga tudo em "Rua" para não
// perder a informação já cadastrada.
function _parseEnderecoCliente(endereco) {
  const vazio = { cep:'', rua:'', numero:'', bairro:'', cidade:'', uf:'' };
  if (!endereco) return vazio;
  // Formato esperado: "Rua, Número - Bairro, Cidade/UF - CEP 00000-000"
  const m = endereco.match(/^(.*?),\s*(.*?)\s*-\s*(.*?),\s*(.*?)\/(\w{2})(?:\s*-\s*CEP\s*([\d-]+))?$/i);
  if (m) {
    return { rua: m[1].trim(), numero: m[2].trim(), bairro: m[3].trim(), cidade: m[4].trim(), uf: m[5].trim().toUpperCase(), cep: (m[6]||'').trim() };
  }
  return { ...vazio, rua: endereco };
}

// Monta a string única salva no banco (coluna "endereco" continua igual — sem migração de banco)
function montarEnderecoCliente() {
  const cep     = _c(gv('form-cli-cep', ''), 10);
  const rua     = _c(gv('form-cli-rua', ''), 100);
  const numero  = _c(gv('form-cli-num', ''), 20);
  const bairro  = _c(gv('form-cli-bairro', ''), 60);
  const cidade  = _c(gv('form-cli-cidade', ''), 60);
  const uf      = _c(gv('form-cli-uf', ''), 2).toUpperCase();
  const partes = [];
  if (rua)    partes.push(numero ? `${rua}, ${numero}` : rua);
  if (bairro) partes.push(cidade ? `${bairro}, ${cidade}${uf ? '/' + uf : ''}` : bairro);
  else if (cidade) partes.push(`${cidade}${uf ? '/' + uf : ''}`);
  let end = partes.join(' - ');
  if (cep) end += (end ? ' - ' : '') + `CEP ${cep}`;
  return _c(end, 200);
}

async function buscarCEP() {
  const cepEl = document.getElementById('form-cli-cep');
  const st    = document.getElementById('form-cli-cep-status');
  if (!cepEl) return;
  const cep = cepEl.value.replace(/\D/g, '');
  if (cep.length !== 8) return;
  if (st) { st.textContent = 'Buscando CEP...'; st.style.color = 'var(--text-3)'; }
  try {
    const resp = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
    const data = await resp.json();
    if (data.erro) {
      if (st) { st.textContent = 'CEP não encontrado — preencha manualmente'; st.style.color = 'var(--red)'; }
      return;
    }
    const ruaEl = document.getElementById('form-cli-rua');
    const bEl   = document.getElementById('form-cli-bairro');
    const cidEl = document.getElementById('form-cli-cidade');
    const ufEl  = document.getElementById('form-cli-uf');
    const numEl = document.getElementById('form-cli-num');
    if (ruaEl && data.logradouro) ruaEl.value = data.logradouro;
    if (bEl   && data.bairro)     bEl.value   = data.bairro;
    if (cidEl) cidEl.value = data.localidade || '';
    if (ufEl)  ufEl.value  = data.uf || '';
    if (st) { st.textContent = 'CEP encontrado ✓'; st.style.color = 'var(--green)'; }
    // Foca no próximo campo que o usuário precisa preencher (número, ou rua se a cidade não tiver logradouro)
    if (numEl && data.logradouro) numEl.focus();
    else if (ruaEl && !data.logradouro) ruaEl.focus();
  } catch(e) {
    if (st) { st.textContent = 'Sem conexão para buscar o CEP — preencha manualmente'; st.style.color = 'var(--red)'; }
  }
}
...
function editarCliente(id) {
  ...
  const end = _parseEnderecoCliente(c.endereco || '');
  const setV = (id, v) => { const el = document.getElementById(id); if (el) el.value = v || ''; };
  setV('form-cli-cep', end.cep); setV('form-cli-rua', end.rua); setV('form-cli-num', end.numero);
  setV('form-cli-bairro', end.bairro); setV('form-cli-cidade', end.cidade); setV('form-cli-uf', end.uf);
  const st = document.getElementById('form-cli-cep-status'); if (st) st.textContent = '';
...
async function salvarCliente() {
  ...
  const d  = {
    ...
    endereco:  montarEnderecoCliente(),
